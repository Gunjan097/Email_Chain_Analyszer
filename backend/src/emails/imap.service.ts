        ...
        this.imap.search(crit, (err, results) => {
            ...
            f.on('message', (msg, seqno) => {
                ...
                msg.on('body', async (stream) => {
                    try {
                        const readable = new Readable().wrap(stream as any); // Cast and wrap correctly
                        ...
                        const receivingChain = this.normalizeReceivingChain(Array.from(parsed.headerLines || []));
                        ...
                        const doc = {
                            ...
                            to: (parsed.to as any)?.text || (parsed.to as any)?.value?.map((v: any) => v.address).join(', ') || '',
